...
export default function Navbar() {
  ...
  return (
    <nav
      className="sticky top-0 z-50 w-full transition-all duration-500 bg-(--cream)"
      style={{
        borderBottom: `1px solid ${scrolled ? "var(--gold)" : "transparent"}`,
        boxShadow: scrolled ? "0 4px 30px rgba(17,22,19,0.06)" : "none",
      }}
    >
          ...
          <div className="hidden lg:flex items-center gap-8 flex-1">
            {LEFT_LINKS.map((link) => (
              <Link
                key={link.href}
                href={link.href}
                className="text-xs uppercase tracking-[0.18em] text-(--forest) hover:text-(--gold) transition-colors font-medium"
              >
                {link.label}
              </Link>
            ))}
      ...
      >
        <div className="flex items-center justify-between px-6 h-20 border-b border-(--gold)">
          <span className="text-sm uppercase tracking-[0.2em] text-(--forest)">
            Menu
          </span>
          <button
        ...
        <div className="flex flex-col px-6 py-10 gap-8">
          {NAV_LINKS.map((link) => (
            <Link
              ...
              href={link.href}
              onClick={() => setMobileOpen(false)}
              className="text-(--forest) text-lg uppercase tracking-[0.12em] hover:text-(--gold) transition-colors"
            >
              {link.label}
            </Link>
          ))}
